...
def CreateLayerHistory(args, is_encoder):
    ...
    elif history_type == "rk_predicotor_multistep_corrector":
        return RKPredictorMultistepCorrectorHistory(args, is_encoder)
    elif history_type == "Adams-Bashforth-Moulton":
        return AdamsBashforthMoultonLayerHistory(args, is_encoder)
    else:
        raise ValueError
...
class RKPredictorMultistepCorrectorHistory(BaseLayerHistory):
    ...
    """
    def __init__(self, args, is_encoder):
        super(RKPredictorMultistepCorrectorHistory, self).__init__(args, is_encoder)
        self.sum = None
        self.count = 0
        self.layer_num = 1 + (args.encoder_layers if is_encoder else args.decoder_layers)
        self.weight = nn.Parameter(torch.Tensor(self.layer_num, self.layer_num).fill_(1.0).tril())
